...
export const chatGptService =() =>{
    ...
    const GET_RECIPE_WEHATER = async (data:any,token:any) =>{
        console.log('GET_RECIPE_WEHATER TOKEN DATA LOG\n',token)
        ...
        // const message = {  prompt: "날씨가 맑은데 메뉴를 추천해줘 답은 8개정도를 json객체 데이터 형식으로 {menu:[]}와 같이 단어로만 말해줘"}
    ...
    const GET_REFIGATOR_RECOMMEND = async({data,token}:any) => {
    ...
    const GET_NEW_RECIPE = async(token:any) => {
    ...
    const GET_NEW_KEYWORD_RECIPE = async({data,token}:any) => {
        const message = {prompt:`다시 ${data} 메뉴를 추천해줘 답은 8개정도를 json객체 데이터 형식으로 단어로만 말해줘`}
        const server = `${path}/openai/chat`
    ...
    const GET_KEYWORD_RECIPE = async(token:any) => {
        const message = {  prompt: "한식 일식 중식 양식 메뉴를 추천해줘 답은 각각 8개정도를 json객체 데이터 형식으로 단어로만 말해주고 title은 kr jp cn usa 이렇게 해줘"}
        const server = `${path}/openai/chat`
